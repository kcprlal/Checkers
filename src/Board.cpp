...
void Board::makeMove(bool xplays, int moveFrom, int moveTo) {
    onSiteLeft = onSiteRight = onSiteLeftTake = onSiteRightTake = false;

    if (moveFrom % 8 == 0) onSiteLeft = true;
    else if (moveFrom % 8 == 7) onSiteRight = true;
    else if (moveFrom % 8 == 6) onSiteRightTake = true;
    else if (moveFrom % 8 == 1) onSiteLeftTake = true;

    if (xplays) {
        if ((moveTo >= 0 && moveTo < 64 && moveFrom >= 0 && moveFrom < 64) && field[moveFrom] == 'x') {
            if (checkIfLegal(0, moveFrom, moveTo, onSiteLeft, onSiteRight)) {
                field[moveFrom] = ' ';
                field[moveTo] = (moveTo > 55) ? 'X' : 'x';
...
bool Board::checkForWin(bool xplays) {
    return xplays ? (Opawns < 1) : (Xpawns < 1);
}
